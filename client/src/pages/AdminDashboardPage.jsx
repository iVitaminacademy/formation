...
export default function AdminDashboardPage() {
  ...
  const kpis = useMemo(() => {
    const parentAccounts = profiles.filter(p => p.role === 'parent')
    const kids = profiles.filter(p => p.role === 'kid')
    const admins = profiles.filter(p => p.role === 'admin')
    const kidsLinked = new Set(links.map(l => l.child_id))
    const independentStudents = kids.filter(k => !kidsLinked.has(k.id))
    const completedProgress = progress.filter(p => p.completed)
    const avgScore = progress.length ? Math.round(progress.reduce((sum, row) => sum + (Number(row.score) || 0), 0) / progress.length) : 0
    const completionRate = progress.length ? Math.round((completedProgress.length / progress.length) * 100) : 0
    const activeParents = parentAccounts.filter(parent => links.some(link => link.parent_id === parent.id)).length
    const averageStreak = kids.length ? Math.round(kids.reduce((sum, kid) => sum + (kid.streak_days || 0), 0) / kids.length) : 0
    ...
    return {
      ...
      activeParents,
      independentStudents: independentStudents.length,
      linkedStudents: kidsLinked.size,
      totalProgress: progress.length,
      completionRate,
      avgScore,
      averageStreak,
      completedProgress: completedProgress.length,
      avgAttempts: progress.length ? Math.round(progress.reduce((sum, row) => sum + (Number(row.attempts) || 0), 0) / progress.length) : 0,
  ...
  const parentsWithChildren = useMemo(() => {
    return profiles.filter(p => p.role === 'parent').map(parent => {
      const childIds = links.filter(link => link.parent_id === parent.id).map(link => link.child_id)
      const childrenRows = profiles.filter(p => childIds.includes(p.id))
      return { ...parent, children: childrenRows }
    })
  }, [links, profiles])
  ...
  const filteredParentsWithChildren = useMemo(() => {
    ...
    return parentsWithChildren.filter(parent => {
      const parentMatch = matchesSearch(parent.name, parentSearch) || matchesSearch(parent.email, parentSearch)
      const childMatch = parent.children.some(child =>
        matchesSearch(child.name, parentSearch) ||
        matchesSearch(child.email, parentSearch) ||
        matchesSearch(child.link_code, parentSearch),
      )
  ...
  const filteredIndependentStudents = useMemo(() => {
    ...
    return independentStudents.filter(student =>
      matchesSearch(student.name, parentSearch) ||
      matchesSearch(student.email, parentSearch) ||
      matchesSearch(student.link_code, parentSearch),
    )
  ...
  const selectedChildProgress = useMemo(() => {
    ...
    return progress
      .filter(row => row.user_id === selectedChild.id)
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await updateProfileByAdmin(selectedProfile.id, {
        ...
        grade: profileForm.role === 'kid' ? Number(profileForm.grade) : null,
  ...
  const handleAddLink = async () => {
    ...
    setSaving(true)
    try {
      await linkParentChild(newParentId, newChildId)
      setRefreshTick(t => t + 1)
      setNewChildId('')
      setNewParentId('')
    } catch (err) {
      setLinkError(err.message || 'Could not create that relationship.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleRemoveLink = async (parentId, childId) => {
    ...
    try {
      ...
      setLinkError(err.message || 'Could not remove that relationship.')
    } finally {
      setSaving(false)
    }
  }
      ...
      <header className="sticky top-0 z-20 border-b backdrop-blur" style={{ backgroundColor: theme.surface + 'EE', borderColor: theme.border }}>
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div className="min-w-0">
            <div className="text-xs font-extrabold uppercase tracking-[0.3em]" style={{ color: theme.subtext }}>
              Admin Dashboard
            </div>
              ...
              action={
                <div className="flex w-full items-center gap-2 sm:w-auto">
                  <input
                    ...
                    className="w-full rounded-xl border px-3 py-2 text-sm outline-none sm:w-56"
                    style={{ borderColor: theme.border, backgroundColor: theme.surface, color: theme.text }}
                  />
                </div>
              }
              ...
              <div className="space-y-4">
                {filteredParentsWithChildren.length === 0 ? (
                  <p className="text-sm" style={{ color: theme.subtext }}>
                    No matching parents or linked students.
                  </p>
                ) : (
                  filteredParentsWithChildren.map(parent => (
                    <div
                      key={parent.id}
                      className="w-full rounded-2xl border p-4"
                      style={{ borderColor: theme.border, backgroundColor: theme.surfaceSoft }}
                    >
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="min-w-0">
                          <div className="break-words font-extrabold">{parent.name || 'Unnamed parent'}</div>
                          <div className="break-words text-xs font-medium" style={{ color: theme.subtext }}>
                            {parent.email || '—'}
                          </div>
                        </div>
                        <Pill tone="green" theme={theme}>
                          ...
                          parent.children.map(child => (
                            <div
                              ...
                              onClick={() => {
                                ...
                                handleSelectProfile(child.id)
                              }}
                              className="cursor-pointer rounded-xl border p-3 text-left transition hover:-translate-y-0.5"
                              style={{ borderColor: theme.border, backgroundColor: theme.surface }}
                            >
                              <div className="flex items-start gap-3">
                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-lg" style={{ backgroundColor: theme.primarySoft }}>
                                  {child.avatar || '🧒'}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <div className="truncate font-bold">{child.name || 'Unnamed student'}</div>
                                  <div className="text-xs" style={{ color: theme.subtext }}>
                                    Grade {child.grade ?? '—'} · 🔥 {child.streak_days ?? 0}
                                  </div>
                                </div>
                                <button
                                  type="button"
                                  onClick={e => {
                                    e.stopPropagation()
                                    handleRemoveLink(parent.id, child.id)
                                  }}
                                  className="shrink-0 rounded-lg border px-2 py-1 text-xs font-bold"
                                  style={{ borderColor: theme.border, backgroundColor: theme.surface, color: theme.danger }}
                                >
                                  Unlink
                                </button>
                              </div>
            ...
            <SectionCard
              ...
              action={
                <input
                  ...
                  className="w-full rounded-xl border px-3 py-2 text-sm outline-none sm:w-44"
                  style={{ borderColor: theme.border, backgroundColor: theme.surface, color: theme.text }}
                />
              }
            >
                  ...
                  {filteredIndependentStudents.map(student => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        handleSelectProfile(student.id)
                      }}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs" style={{ color: theme.subtext }}>
                          Grade {student.grade ?? '—'} · link code {student.link_code || '—'}
                        </div>
                      </div>
                    </button>
                  ))}
            ...
            <SectionCard title="Relationship manager" theme={theme}>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                  ...
                  {profiles.filter(p => p.role === 'kid').map(child => (
                    <option key={child.id} value={child.id}>
                      {child.name || child.email || child.id}
                    </option>
                  ))}
                </select>
              ...
              {linkError && (
                <p className="mt-3 text-sm font-semibold" style={{ color: theme.danger }}>
                  {linkError}
                </p>
              )}
              ...
              <div className="mt-5 overflow-x-auto rounded-2xl border" style={{ borderColor: theme.border }}>
                <table className="min-w-[640px] w-full text-left text-sm">
                  ...
                  <tbody>
                    {links.length === 0 ? (
                      <tr>
                        <td className="px-4 py-4" colSpan="4" style={{ color: theme.subtext }}>
                      ...
                      links.map(row => (
                        <tr key={`${row.parent_id}-${row.child_id}`} className="border-t" style={{ borderColor: theme.border }}>
                          ...
                          <td className="px-4 py-3">
                            <button
                              onClick={() => handleRemoveLink(row.parent_id, row.child_id)}
                  ...
                  >
                    {profiles.map(p => (
                      <option key={p.id} value={p.id}>
                        {p.name || p.email || p.id} · {p.role}
                      </option>
                    ))}
                  </select>

                  <label className="block text-xs font-bold uppercase tracking-wide" style={{ color: theme.subtext }}>
                    Name
                  </label>
                  <input
              ...
              {selectedChild ? (
                <div className="space-y-3">
                  <div className="rounded-xl border p-3" style={{ borderColor: theme.border, backgroundColor: theme.surfaceSoft }}>
                    ...
                    <div className="text-xs" style={{ color: theme.subtext }}>
                      Showing progress rows for this student
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <input
                      ...
                      placeholder="Lesson title or ref"
                  ...
                  <div className="max-h-56 overflow-auto rounded-2xl border" style={{ borderColor: theme.border }}>
                    {selectedChildProgress.length === 0 ? (
                      <div className="p-4 text-sm" style={{ color: theme.subtext }}>
                        No progress rows yet for this student.
                      </div>
                    ) : (
                      selectedChildProgress
                        .filter(row => {
                          const lessonMeta = lessonIndex.get(Number(row.lesson_ref))
                          const displayTitle = lessonMeta?.lessonTitle || `Lesson ${row.lesson_ref}`
                          return (
                            matchesSearch(row.lesson_ref, progressForm.lessonRef) ||
                            matchesSearch(displayTitle, progressForm.lessonRef) ||
                        ...
                        .map(row => {
                          const lessonMeta = lessonIndex.get(Number(row.lesson_ref))
                          const displayTitle = lessonMeta?.lessonTitle || `Lesson ${row.lesson_ref}`
                          const topicName = lessonMeta?.topicName || 'Unknown topic'
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  <div className="text-xs" style={{ color: theme.subtext }}>
                                    {topicName} · Lesson ref {row.lesson_ref}
                                  </div>
                                  <div className="mt-1 text-xs" style={{ color: theme.subtext }}>
                                    Score: {formatScore(row.score)} · Attempts: {row.attempts ?? 1} · {row.completed ? 'Completed' : 'In progress'}
                                  </div>
                                </div>
                                <div className="shrink-0 text-right text-xs" style={{ color: theme.subtext }}>
                                  <div className="font-bold uppercase tracking-wide">Last updated</div>
                    ...
                    {selectedProgressRowId ? (
                      <div className="mt-3 space-y-3">
                        <div className="text-sm font-semibold break-words">
                          {lessonIndex.get(Number(progress.find(r => r.id === selectedProgressRowId)?.lesson_ref))?.lessonTitle || 'Selected lesson'}
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              ...
                              style={{ borderColor: theme.border, backgroundColor: theme.surface, color: theme.text }}
                            />
                            <div className="mt-1 text-[11px]" style={{ color: theme.subtext }}>
                              Percentage score shown as a percent, for example 3% or 100%.
                            </div>
                          </div>
                          <div>
                            <label className="block text-xs font-bold uppercase tracking-wide" style={{ color: theme.subtext }}>
                      ...
                      <p className="mt-2 text-sm" style={{ color: theme.subtext }}>
                        Select a progress row above to edit score, attempts, and completion status.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <p className="text-sm" style={{ color: theme.subtext }}>
                  Select a student to update their progress.
                </p>
              )}
            </SectionCard>
            ...
            <SectionCard title="Database overview" theme={theme}>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border p-4" style={{ borderColor: theme.border, backgroundColor: theme.surfaceSoft }}>
                  ...
                  <div className="mt-2 text-2xl font-extrabold">{roleCounts.kid}</div>
                </div>
                <div className="rounded-2xl border p-4" style={{ borderColor: theme.border, backgroundColor: theme.surfaceSoft }}>
                  <div className="text-xs font-extrabold uppercase tracking-widest" style={{ color: theme.subtext }}>
                    Parents
                  </div>
                  <div className="mt-2 text-2xl font-extrabold">{roleCounts.parent}</div>
                </div>
                <div className="rounded-2xl border p-4" style={{ borderColor: theme.border, backgroundColor: theme.surfaceSoft }}>
                  <div className="text-xs font-extrabold uppercase tracking-widest" style={{ color: theme.subtext }}>
                    Admins
                  </div>
                  <div className="mt-2 text-2xl font-extrabold">{roleCounts.admin}</div>
                </div>
              </div>
            </SectionCard>
          </div>
